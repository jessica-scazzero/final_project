...
summarize(count = n()) %>% 
  ungroup() %>%
  group_by(year) %>% 
  mutate(relative = count/n())

ggplot(data = cash_by_year) + 
geom_col(mapping = aes(x = year, y = relative)) 

+ geom_text(aes(x = year, label = paste0(relative,'%')),
...
ifelse(pi == 2, 2,
ifelse(pi == 3, 3,
ifelse(pi == 4, 4, 5))))) %>% group_by(year, pi_2) %>% count() %>% ungroup() %>% group_by(year) %>% mutate(relative = n/sum(n)) %>% view()


#group_by(year) %>%
 summarize(percent_cash = sum(pi == 1)/)

ggplot(data = cash_by_year) + 
geom_col(mapping = aes(x = year, y = percent_cash))
  
  
cash_by_year_2 <- merged_2 %>%
  drop_na(year) %>%
  drop_na(pi) %>%
   mutate(pi_2 = ifelse(pi == 1, 1,
                ifelse(pi == 2, 2,
                ...
                ifelse(pi == 4, 4, 5))))) %>%
  group_by(year, pi_2) %>%
  summarise('number'=sum(pi_2)) %>%
  ungroup() %>%
  mutate('relative'=unlist(by(data = number, INDICES = year, 
                              FUN = function(x) round(x/sum(x)*100, digits = 1))))
  
ggplot(merged_2, )


final_year_graph <- merged_2 %>%
  drop_na(year) %>%
  drop_na(pi) %>%
  mutate(pi_2 = ifelse(pi == 1, 1,
                ifelse(pi == 3, 2,
                ifelse(pi == 4, 3, 4)))) %>% group_by(year, pi_2) %>% count() %>% ungroup() %>% group_by(year) %>% mutate(relative = n/sum(n))

ggplot(data = final_year_graph) + 
geom_col(mapping = aes(x = year, y = relative, fill = as.factor(pi_2))) 
